...
class TasksSet:
    ...
    def __init__(self,Parameters,transforms_image):
        ...
        self.root1 = os.path.join(self.root, 'images_background')
        self.root2 = os.path.join(self.root, 'images_evaluation')
        self.arg = Parameters
        ...
        for temp_fold in [self.root1,self.root2]:
            for fold in os.listdir(temp_fold):   #
                character_idx = self.Index_Len
                self.Index_Len += 1
                self.Path_dict[character_idx] = []
                self.Index_dict[character_idx] = 0  #记录的index
                data_point_index = 0  # 记录数据集的数量
                self.Name_dict[character_idx] = fold
                for filename in os.listdir(os.path.join(temp_fold, fold)):   #
                    self.Path_dict[character_idx].append(os.path.join(os.path.join(temp_fold, fold), filename))
                    data_point_index += 1
