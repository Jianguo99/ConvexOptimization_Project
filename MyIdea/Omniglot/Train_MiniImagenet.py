...
from MiniImagenet import MiniInagenetFolder, split_omniglot, ImageCache, transform_image, transform_label
from utils import find_latest_file,set_learning_rate,get_loss
...
def do_learning(net, optimizer, train_iter, iterations):
    ...
    for iteration in range(iterations):
        # Sample minibatch
        data, labels = Variable_(train_iter.__next__())
...
def do_evaluation(net, test_iter, iterations):
    ...
    for iteration in range(iterations):
        # Sample minibatch
        data, labels = Variable_(test_iter.__next__())
...
parser.add_argument('--validate-every', default=200, type=int, help='Meta-evaluation every ... base-tasks')
parser.add_argument('--input', default='/home/huang/Desktop/Hw/ConvexOptimization/reptile-pytorch/MiniImagenet', help='Path to omniglot dataset')
...
omniglot = MiniInagenetFolder(args.input, size=(32, 32), cache=ImageCache(),
...
torch.cuda.empty_cache()
meta_net = MiniImagenetModel(args.classes)
if args.cuda:
    meta_net.cuda()
...
for meta_iteration in tqdm.trange(args.start_meta_iteration, args.meta_iterations):
    ...
    # the value is decreasing over time
